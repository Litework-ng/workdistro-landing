...
async function submitWaitlist(data: Record<string, unknown>): Promise<WaitlistResult> {
  try {
    const res = await fetch("https://workdistro-backend-1.onrender.com/api/wait-list/", {
      method: "POST",
          ...
          <AnimatePresence mode="wait">
            ...
            >
              <Image
                ...
                priority
              />
              {/* Soft ring to make image feel elevated */}
          ...
          <p className="text-gray-400 mb-8">
            ...
            onSubmit={async (e) => {
             ...
             const result = await submitWaitlist(data);
              if (result.success) {
                setAlert({ show: true, message: "🎉 You’ve been added to the waitlist successfully!", type: "success" });
                form.reset();
              } else {
            ...
            <p className="text-sm text-gray-400">
             ...
             <form
              ...
              onSubmit={async (e) => {
               ...
               if (!emailInput?.value) {
                setAlert({ show: true, message: "⚠️ Please enter a valid email.", type: "info" });
               ...
               const result = await submitWaitlist(data);
                if (result.success) {
                  setAlert({ show: true, message: "🎉 You’ve been subscribed to the newsletter!", type: "success" });
                  form.reset();
                } else {
                  setAlert({ show: true, message: `❌ Error: ${result.message}`, type: "error" });
                }
